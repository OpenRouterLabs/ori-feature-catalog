 ...
 *
 * The handle lives in a `globalThis` slot, not in module scope. Ori rebuilds
 * this feature per load — `importFreshModule` bundles it to a temp file and
 * imports that — so module scope is per-load, and `start` sets a binding one
 * copy owns while the route handler reads `undefined` in another. Every
 * request then answers 503 while boot reports the surface live. That shipped
 * once, in #31. Nothing inside the bundle can bridge it: not a closure, not a
 * leaf module. Only something above it, which is `globalThis`.
 */
...
const withRuntime = (reach: (runtime: SlackRuntime) => Answer): Answer => {
  const runtime = slackRuntime.read();
...
const makeLazySlackChat = (): ChatContribution => {
  ...
  return {
    ...
    async start(bridge: Chat) {
      ...
      uninstall = slackRuntime.install(active);
    },
