...
/** Slack collapses on rendered height, so that is what the tests measure. */
const BUDGET = 6;
...
describe("the work log carries the agent's own words", () => {
  test("logs what it said, never the tool calls between", () => {
    ...
    // The sentence written before reaching for the tool is the whole signal.
  ...
  test("keeps a short tail, so Slack never collapses the message", () => {
    // Past roughly this much rendered height Slack hides the rest behind
    // "Show more" — and since every flush edits the message, it snaps shut
    // again the moment the reader expands it.
    let state = initialRunState(0);
  ...
  test("budgets by wrapped height, not by number of entries", () => {
    // Five long sentences wrap to eight visual lines in a thread pane, which
    // is what put the message back under the fold while the entry count still
    // looked fine.
    ...
    expect(wrappedHeight(rendered)).toBeLessThanOrEqual(BUDGET);
    // Long lines mean FEWER of them survive; short ones mean more.
    expect(rendered.split("\n").length).toBeLessThan(3);
  });

  test("a restatement does not cost a line", () => {
    // An agent narrating its own progress repeats itself constantly, and each
    // near-repeat pushes a real update out of the tail.
    let state = initialRunState(0);
  ...
  test("interleaves posted statuses with narration in order", () => {
    // This is what makes it read as a colleague working rather than a spinner.
    let state = initialRunState(0);
  ...
  test("flattens a multi-line block onto one line", () => {
    // Narration arrives as a paragraph; four lines of it would crowd out
  ...
  test("lets a normal sentence through untouched", () => {
    // Slack wraps a long line perfectly well. Cutting one mid-word reads as
    // damage rather than as brevity, and these are sentences now.
    const sentence =
  ...
  test("shows the sentence still being written, so the text grows", () => {
    // Without the in-flight block the message sat still through a whole
    // paragraph and then jumped, because only a tool call closes a block.
    ...
    // The trailing mark says it is mid-sentence: without it a line caught
    // between words — "Mapping the current tu" — reads as damage.
    expect(early).toBe("I'll take another…");
    // No mark once the sentence has landed: "the repo.…" is exactly the
    // doubled punctuation this is meant to avoid looking like.
  ...
  test("does not repeat a block once the tool call closes it", () => {
    // The in-flight line becomes a log line; rendering both would double it.
  ...
  test("windows a long in-flight line from the end, so it keeps moving", () => {
    // Truncating from the front freezes it after the first screenful, which
    // is the opposite of looking live.
  ...
  test("is plain prose, not a code block", () => {
    // A fixed-width grey box reads as terminal output to be parsed rather
  ...
  test("renders nothing at all before the agent has said anything", () => {
    // An empty code block is a black rectangle where progress should be.
...
describe("the window is fixed: the top scrolls off, the bottom keeps moving", () => {
  test("the in-flight line is paid for out of the same budget", () => {
    // Appending it on top of an already-full log pushed the render over the
    // fold, which re-collapses on every edit.
    let state = initialRunState(0);
  ...
  test("a single line longer than the whole budget still renders", () => {
    // Otherwise one pasted paragraph empties the message entirely.
...
describe("an ending that is not an answer still reports the work", () => {
  test.each([RunPhase.TimedOut, RunPhase.Cancelled, RunPhase.Failed])(
    ...
    (phase) => {
      // The progress message is deleted when the turn ends, so without this
      // the log dies with it — an hour of real work rendered as "Timed out."
      // and nothing else. No answer AND no record is the worst outcome.
  ...
  test("a run that did nothing says only how it ended", () => {
    // An empty "What I got done" heading is worse than no heading.
    const rendered = renderRunState({
      ...initialRunState(0),
  ...
  test("a successful turn reports the answer, not the narration", () => {
    // On success the answer IS the record; repeating the log under it is the
    // "bash ×36" mistake in another costume.
    const rendered = renderRunState({
...
describe("a sentence is never split across two log lines", () => {
  test("a tool call landing mid-sentence does not close the block", () => {
    // Deltas arrive in chunks and a tool call lands wherever it lands.
    // Closing unconditionally froze half a sentence into the log and started
    // the rest as a new line — "Mapping the current tu" above "rn path."
  ...
  test("an unfinished sentence is still visible while it is being written", () => {
    // It must not vanish just because it has not landed yet.
    let state = withProse(initialRunState(0), "Mapping the current tu");
    state = withTool(state, "bash");
